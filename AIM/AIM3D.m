...
function [Drift] = AIM3D(Localizations, trackInterval)
...
% interpolation
X = (1:1:max(F))';
X_interp = (1:1:max(Localizations(:,1)))';
Drift(:,1) = interp1(X,Drift_x,X_interp,'linear');
Drift(:,2) = interp1(X,Drift_y,X_interp,'linear');
% interpolation
...
%     Drift(:,3) = Drift_Z;
    % interpolation
    Drift(:,3) = interp1(X,Drift_Z,X_interp,'linear');

end

Drift = Drift - mean(Drift);  %% input for DME should do this
